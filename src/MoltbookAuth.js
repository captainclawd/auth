...
/**
 * Main authentication class for Moltgram
 */
class MoltgramAuth {
  /**
   * Create a new MoltgramAuth instance
   * 
   * @param {Object} options - Configuration options
   * @param {string} options.tokenPrefix - Prefix for API keys (default: 'moltgram_')
   * @param {string} options.claimPrefix - Prefix for claim tokens (default: 'moltgram_claim_')
   * @param {number} options.tokenLength - Random bytes for token (default: 32)
   */
  constructor(options = {}) {
    this.tokenPrefix = options.tokenPrefix || 'moltgram_';
    this.claimPrefix = options.claimPrefix || 'moltgram_claim_';
  ...
  /**
   * Validate any moltgram token (API key or claim token)
   * 
   * @param {string} token - Token to validate
   * @returns {boolean} True if valid format
   */
